...
static void GPS_DMA_Config(void)
{
  ...
  __HAL_LINKDMA(&GPS_UartHandle, hdmatx, DMA_Handle);
	/*配置GPS使用的DMA中断优先级*/
...
void GPS_ProcessDMAIRQ(void)
{
  ...
  if(__HAL_DMA_GET_FLAG(&DMA_Handle,GPS_DMA_IT_HT) )         /* DMA 半传输完成 */
  {
    GPS_HalfTransferEnd = 1;                //设置半传输完成标志位
    __HAL_DMA_CLEAR_FLAG (&DMA_Handle,GPS_DMA_IT_HT); 
  }
  else if(__HAL_DMA_GET_FLAG(&DMA_Handle,GPS_DMA_IT_TC))     /* DMA 传输完成 */
  {
    GPS_TransferEnd = 1;                    //设置传输完成标志位
    __HAL_DMA_CLEAR_FLAG(&DMA_Handle,GPS_DMA_IT_TC);
