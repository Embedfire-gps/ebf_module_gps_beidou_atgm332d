...
void nmea_decode_test(void)
{
    ...
    LCD_SetColors(LCD_COLOR_RED,LCD_COLOR_BLACK);
  	...
  	LCD_DisplayStringLine_EN_CH(1,(uint8_t *)" Wildfire STM32-H750");
    LCD_DisplayStringLine_EN_CH(2,(uint8_t *)"  GPS module");
	    //链接驱动器，创建盘符
    ...
    while(!f_eof(&log_file))
    {
        ...
        LCD_DisplayStringLine_EN_CH(6,(uint8_t *)str_buff);
      
        /* 纬度 经度*/
        sprintf(str_buff," latitude :%.6f ", deg_lat);
        LCD_DisplayStringLine_EN_CH(7,(uint8_t *)str_buff);
        
        sprintf(str_buff," longitude :%.6f",deg_lon);
        LCD_DisplayStringLine_EN_CH(8,(uint8_t *)str_buff);
        
        /* 正在使用的卫星 可见的卫星*/
        sprintf(str_buff," GPS  Satellite in use :%2d ", info.satinfo.inuse);
        LCD_DisplayStringLine_EN_CH(9,(uint8_t *)str_buff);    
        
        sprintf(str_buff," GPS Satellite in view :%2d", info.satinfo.inview);
        LCD_DisplayStringLine_EN_CH(10,(uint8_t *)str_buff);    

        /* 正在使用的卫星 可见的卫星*/
        sprintf(str_buff," BDS  Satellite in use :%2d ", info.BDsatinfo.inuse);
        LCD_DisplayStringLine_EN_CH(11,(uint8_t *)str_buff);    
        
       sprintf(str_buff," BDS Satellite in view :%2d", info.BDsatinfo.inview);
        LCD_DisplayStringLine_EN_CH(12,(uint8_t *)str_buff);    
        
        /* 海拔高度 */
        sprintf(str_buff," Altitude:%4.2f m", info.elv);
        LCD_DisplayStringLine_EN_CH(13,(uint8_t *)str_buff);
        
        /* 速度 */
        sprintf(str_buff," speed:%4.2f km/h", info.speed);
        LCD_DisplayStringLine_EN_CH(14,(uint8_t *)str_buff);
        
        /* 航向 */
        sprintf(str_buff," Track angle:%3.2f deg", info.direction);
        LCD_DisplayStringLine_EN_CH(15,(uint8_t *)str_buff);
...
int nmea_decode_test(void)
{
    ...
    nmea_property()->error_func = &error;
    nmea_property()->info_func = &gps_info;
 LCD_DisplayStringLine_EN_CH(2,(uint8_t *)"  RUNNING11111");
    /* 初始化GPS数据结构 */
    nmea_zero_INFO(&info);
    nmea_parser_init(&parser);
 LCD_DisplayStringLine_EN_CH(2,(uint8_t *)"  RUNNING22222");
    while(1)
    {
			
      if(GPS_HalfTransferEnd)     /* 接收到GPS_RBUFF_SIZE一半的数据 */
      {
				 LCD_DisplayStringLine_EN_CH(3,(uint8_t *)"  RUNNING33333");
        /* 进行nmea格式解码 */
        nmea_parse(&parser, (const char*)&gps_rbuff[0], HALF_GPS_RBUFF_SIZE, &info);
         LCD_DisplayStringLine_EN_CH(3,(uint8_t *)"  COMPLETE");
