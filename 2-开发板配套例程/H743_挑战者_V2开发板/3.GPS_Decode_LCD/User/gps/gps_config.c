...
static void GPS_USART_Config(void)
{
    ...
    /* 配置串口3时钟源*/
		...
		RCC_PeriphClkInit.Usart16ClockSelection = RCC_USART234578CLKSOURCE_PLL2;
    ...
    GPS_UartHandle.Init.BaudRate = 9600;
...
static void GPS_DMA_Config(void)
{
  ...
  /*usart3 rx对应dma2，通道4，数据流7*/	
  DMA_Handle.Init.Request = DMA_REQUEST_USART3_RX; 
  /*方向：从外设到内存*/		
  ...
  __HAL_LINKDMA(&GPS_UartHandle, hdmarx, DMA_Handle);
	...
	HAL_NVIC_EnableIRQ(GPS_DMA_IRQ);
	 /*配置中断条件*/
	__HAL_DMA_ENABLE_IT(&DMA_Handle,GPS_DMA_IT_TC|GPS_DMA_IT_HT);
...
void GPS_ProcessDMAIRQ(void)
{
		typedef struct
	{
	  ...
	  DMA_Base_Registers *regs = (DMA_Base_Registers *)DMA_Handle.StreamBaseAddress;
  if((__HAL_DMA_GET_IT_SOURCE(&DMA_Handle, GPS_DMA_IT_HT) != RESET) && (GPS_HalfTransferEnd == 0))        /* DMA 半传输完成 */
  {
		
    GPS_HalfTransferEnd = 1;                //设置半传输完成标志位
		 /* Clear the half transfer complete flag */
     regs->IFCR = DMA_FLAG_HTIF0_4 << DMA_Handle.StreamIndex;
  }
  else  if((__HAL_DMA_GET_IT_SOURCE(&DMA_Handle, GPS_DMA_IT_TC) != RESET) && (GPS_TransferEnd == 0) )     /* DMA 传输完成 */
  {
    GPS_TransferEnd = 1;                    //设置传输完成标志位
      /* Clear the transfer complete flag */
    regs->IFCR = DMA_FLAG_TCIF0_4 << DMA_Handle.StreamIndex;
